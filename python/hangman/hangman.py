...
def get_args():
    ...
    parser.add_argument('-l', '--maxlen', help='Max word length', 
                        type=int, default=10)
    parser.add_argument('-n', '--minlen', help='Min word length', 
                        type=int, default=5)
    parser.add_argument('-m', '--misses', help='Max number of misses',
                        type=int, default=10)
...
def main():
    ...
    if min_len > max_len:
        print('--minlen ({}) is greater than --maxlen ({})'.format(min_len, max_len))
        sys.exit(1)
...
def play(state):
    ...
    play({'word': word, 'guessed': guessed, 'num_misses': num_misses,
          'prev_guesses': prev_guesses, 'max_misses': max_misses})

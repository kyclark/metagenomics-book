...
def main():
    ...
    if seqs_per_file < 1:
        die("--sequences_per_file cannot be less than one")

    if not output_format:
        output_format = input_format

    valid_format = set(['fasta', 'fastq'])
    for fmt in [input_format, output_format]:
        if not fmt in valid_format:
            die('Format "{}" not valid, choose from {}'.format(
                fmt, ', '.join(valid_format)))

    num_files = 0
    num_seqs_written = 0
    for i, file in enumerate(files):
        print('{:3d}: {}'.format(i + 1, os.path.basename(file)))
        num_files += 1
